...
function aliasSubstitution(
  ...
  const parameterArguments = type.typeArguments?.params ?? [];
  const next = new Map(base);
  for (const [index, parameter] of parameters.entries()) {
    const argument = parameterArguments[index] ?? parameter.default;
...
// oxlint-disable-next-line complexity -- exhaustive AST-shape dispatch mirrors dictionaryValueTypes' cases; splitting it would scatter one decision across files.
...
// oxlint-disable-next-line complexity -- exhaustive AST-shape dispatch; the branches enumerate the type-node kinds the rule must recognize, not incidental nesting.
...
// oxlint-disable-next-line complexity -- exhaustive AST-shape dispatch over TSType variants; each branch is a distinct case the rule documents.
...
function isBroadMappedKey(
  ...
  const nextVisited = new Set([...visitedAliases, name]);
  return isBroadMappedKey(alias.typeAnnotation, environment, substitutions, nextVisited);
}

// oxlint-disable-next-line complexity -- exhaustive AST-shape dispatch over TSType variants.
