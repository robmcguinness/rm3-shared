...
tester.run('anti-slop/no-shape-in-symbol-names', noForbiddenTermInSymbolNamesRule, {
  ...
  valid: [
    ...
    // A type-position reference into another namespace (e.g. `z.ZodRawShape`)
    // is a TSQualifiedName, not a MemberExpression, but it borrows the name
    // the same way `owner.shape` does at runtime.
    'declare const s: z.ZodRawShape;',
  ],
});
