...
| `no-button-loading-prop`        | An `isLoading` / `isPending` / `loading` / `pending` attribute on `Button`, `InputGroupButton`, `SidebarMenuButton`, `SidebarMenuSubButton`, `AlertDialogAction` or `AlertDialogCancel`. A spread is not read; `data-loading` passes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `prefer-skeleton`               | An `animate-pulse` token (any variant chain) in the `className` of a `div` or `span`. A component element (`Skeleton`, `Badge`) never matches.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `prefer-separator`              | An `<hr>`; or a `div` / `span` with no meaningful children, no opaque children and no spread, carrying a one-edge border token (`border-t`, `border-b-2`, `md:border-l`, `border-s`). `border`, `border-x`, `border-y` and `border-b-0` pass. A markdown renderer's `hr` needs a disable comment.                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `require-native-button-false`   | A `render` whose value is a JSX element naming a host tag other than `button` / `input`, or `InputGroupAddon`, `Badge`, `Item`, `Card`, `Avatar`, `Link`, `NavLink`, on a part built on a Base UI component that takes `nativeButton` (`Button`, `InputGroupButton`, `Toggle`, `ToggleGroupItem`, the Dialog / Sheet / Drawer / AlertDialog / Popover triggers and closes, `DropdownMenuTrigger`, `DropdownMenuSubTrigger`, `MenubarTrigger`, `CollapsibleTrigger`, `AccordionTrigger`, `NavigationMenuTrigger`, `SelectTrigger`, `TabsTrigger`), with no `nativeButton` attribute and no spread. `TooltipTrigger`, `ContextMenuTrigger` and the `SidebarMenu*` buttons have no such prop and are not matched. A function `render` is not read. |
| `no-positioned-input-addon`     | A host element with a `relative` token whose direct elements (through fragments, `&&` and `.map()`) include an `Input` / `Textarea` and another element with an `absolute` token. A component wrapper (`<Field className="relative">`) is not read.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `no-wrapped-trigger`            | A `*Trigger` / `*Close`, `NavigationMenuLink`, `BreadcrumbLink` or `SidebarMenuButton` whose one meaningful child is a `div` / `span` that itself has one element child. Passes when that inner element carries `disabled` (the tooltip-on-disabled-button workaround).                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `prefer-marker`                 | An element (not `Marker`) whose direct children hold two or more `Separator`s with text, an expression or an inline host (`span`, `p`, `small`, `time`, …) between the first and last; or one `Separator` with `flex-1` beside at least one such label. A `Separator` with `orientation="vertical"` (a toolbar divider) makes the element pass; sections (`CardContent`) between separators pass.                                                                                                                                                                                                                                                                                                                                               |
| `require-field-state-pairing`   | A `Field` with `data-invalid` (or `data-disabled`) and at least one control under it (`Input`, `Textarea`, `Select`, `NativeSelect`, `Checkbox`, `RadioGroupItem`, `Switch`, `Slider`, `InputOTP`, `InputGroupInput`, `InputGroupTextarea`), none carrying `aria-invalid` (`disabled`); or such a control carrying the attribute whose nearest `Field` lacks the `data-*`. Presence only. A `Field` with opaque children, a spread, or a component the rule does not know (`EmailInput`) passes.                                                                                                                                                                                                                                                |
| `no-base-api-mismatch`          | Under `base: 'base'` (default): `asChild` anywhere, `SelectContent position`, `type` / `collapsible` on `ToggleGroup` / `Accordion`, a string-literal `value` / `defaultValue` on those two, a one-element array on `Slider`. Under `base: 'radix'`: a `render` set to a JSX element, `SelectContent alignItemWithTrigger`, `multiple` on `ToggleGroup` / `Accordion` / `Select`, `itemToStringValue`, an `Accordion` with no `type` and no spread, a numeric `Slider` value. Either mode: `asChild` and `render` together. `SelectValue placeholder` passes: Base UI's `Select.Value` takes it too. A variable value is not read.                                                                                                              |
